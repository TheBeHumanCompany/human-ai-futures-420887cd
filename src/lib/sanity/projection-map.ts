 ...
 * `guestPhoto` (~75 B/ep once populated) IS included, and that is a layout
 * decision rather than payload generosity: the featured-episode lead renders
 * the guest's portrait — never the square cover artwork or share card, which
 * carry episode text and branding baked into the image. The grid cards still
 * use the `coverArtwork -> shareCard -> default` chain; only the featured
 * lead reads this field.
 *
